...
#
# KEEP TRAINING ON-POLICY. As configured, each rollout is generated by the
# current actor weights and consumed in exactly one optimizer step
# (128 prompts x 8 samples = 1024 = global batch), with weights re-synced to
# SGLang every step. Miles supports overlapping training with sampling (async
# rollout); a train/sample mismatch like that can cause runs to collapse. We
# haven't investigated this carefully — don't enable it unless you know what
# you're doing.
